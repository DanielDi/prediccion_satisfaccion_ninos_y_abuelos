...
varSatisfaccion <- personas %>% select(-ID_Persona,-Clasificación,-SEXO) %>% na.omit() 
M = cor(varSatisfaccion)
...
datos <- transform(datos, SATISFACCION = as.factor(SATISFACCION),
                   SALUD_AUTOPERCIBIDA = as.factor(SALUD_AUTOPERCIBIDA),
                   SEGURIDAD_AUTOPERCIBIDA = as.factor(SEGURIDAD_AUTOPERCIBIDA))



# Convierte los datos a factor para analizar los con regresión.
datos$SATISFACCION = as.factor(datos$SATISFACCION)
datos$SALUD_AUTOPERCIBIDA = as.factor(datos$SALUD_AUTOPERCIBIDA)
datos$SEGURIDAD_AUTOPERCIBIDA = as.factor(datos$SEGURIDAD_AUTOPERCIBIDA)

# Cuenta cuántos 
xtabs(~datos$SATISFACCION + datos$SALUD_AUTOPERCIBIDA, data = datos)
...
# Entrenamiento
mymodel<-glm(datos$SALUD_AUTOPERCIBIDA~datos$SEGURIDAD_AUTOPERCIBIDA+datos$SALUD_AUTOPERCIBIDA,
                        data = train,
                        family = 'binomial')
summary(mymodel)

# Predicción
p1<-predict(mymodel, test,
            type = 'response')
head(p1)

# Crea una tabla con falso positivos (FALLA)
# pre1 <- ifelse(p1 > 0.5, 1, 0)
#table<-table(Prediction = pre1, Actual = train$admit) 
#table

# Y_datos <- subset(datos, select = c(SATISFACCION))
# X_datos <- subset(datos, select = c(SALUD_AUTOPERCIBIDA))


# Normalización
# X_centrada <- scale(X_datos,center = TRUE, scale = TRUE)
# Y_centrada <- scale(Y_datos,center = TRUE, scale = TRUE)


# Determina cuántos K-folds son óptimos
#aprox1_tr_vl<-train(Y_datos$SATISFACCION~.,
#             data       = X_centrada,
#             method     = "knn",
...
# Entrena el knn con 4 K-folds
# modelo_knn_k4 <- knnreg(x=X_centrada,y=Y_datos,k=4)
```
