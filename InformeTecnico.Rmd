...
#                                     NIVEL_DE_EDUCACION, COND_VIDA_DEL_HOGAR, TRABAJO_AUTOPERCIBIDO
save(file="App/modeloSatisfaccion.RData",
                              list=c("lm1"))
```
...
df_ninos <- ninos %>% select(Edad, CANT_PERSONAS_HOGAR, incap, Satisfaccion, PERCAPITA)
...
df_ninos['Beca'] = ninos[,'P8610S1'] 
df_ninos['Medio_transporte'] = addNA(factor(ninos[,'P4693'], labels = c('Carro', 'escolar', 'público','pie', 'Bicicleta', 'Caballo', 'canoa', 'Otro')))
df_ninos['enfermedad_crónica'] =  addNA(factor(ninos[,'P1930'], labels = c(TRUE, FALSE)))
df_ninos['Ingreso'] = ninos[,'P9090']
...
modelo <- ctree(formula = Satisfaccion ~ ., data=train, controls = ctree_control(mincriterion = 0.89))
...
#modelo2 <- ctree(formula = Satisfaccion ~ Hacer_Pricipal + Lugar_de_Trabajo + Medio_transporte +Edad , data=train, controls = ctree_control(mincriterion = 0.7))
