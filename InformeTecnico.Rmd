...
datos <- subset(personas, select = c(SATISFACCION, SALUD_AUTOPERCIBIDA, SEGURIDAD_AUTOPERCIBIDA, NIVEL_DE_EDUCACION))
...
df <- data.frame(X1=c(8), X2=c(8), X3=(8))

#Train
lm1 = lm(formula = Y ~ X1 + X2 + X3, data=train)
glm1 = glm(formula = Y ~ X1 + X2 + X3, data=train)
summary(lm1)
summary(glm1)

p1 <- predict(lm1, newdata=df)
p2 <- predict(glm1, newdata=df)
print(p1)
print(p2)
```
